...
import std.datetime;
    ...
    public class DbfHeader 
    {
        ...
        private int[string] _columnNameIndex = null;
        ...
        public this(int nFieldCapacity)
        {
            _fields.length = nFieldCapacity;
        ...
        /// </summary>
        /// <param name="sName"></param>
        public DbfColumn get(string sName)
        {
        ...
        /// <returns></returns>
        public DbfColumn get(int nIndex)
            {
        ...
        @property uint RecordCount()
        {
             ...
             @property  RecordCount(uint value)
            {
                _numRecords = value;
                //set the dirty bit
                _isDirty = true;
        ...
        @property void Locked(bool value)
        {
           _locked = value;
        }
        ...
        @property bool IsDirty()
           ...
           @property void IsDirty(bool value)
            {
                _isDirty = value;
            }
        ...
        public void Read(File reader)
        {
            ...
            for (int i = 0; i < nNumFields; i++)
            {
                ...
                if (cDbaseType == 'C' || cDbaseType == 'c')
                {
                    ...
                    nFieldLength = cast(int)reader.ReadUInt16();
                }
                else
                {
                    //read field length as an unsigned byte.
                    nFieldLength = cast(int)reader.ReadByte();

                    //read decimal count as one byte
                    nDecimals = cast(int)reader.ReadByte();
